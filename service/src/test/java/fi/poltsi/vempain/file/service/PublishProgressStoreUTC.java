...
class PublishProgressStoreUTC {
	...
	void markScheduled_incrementsAndSetsStatus() {
		...
		assertThat(store.getPerGroupStatus()
		                .get(1L)).isEqualTo(PublishProgressStatusEnum.SCHEDULED);
		assertThat(store.getPerGroupStatus()
		                .get(2L)).isEqualTo(PublishProgressStatusEnum.SCHEDULED);
	}
	...
	void markStarted_incrementsAndSetsStatus() {
		...
		assertThat(store.getPerGroupStatus()
		                .get(1L)).isEqualTo(PublishProgressStatusEnum.STARTED);
	}
	...
	void markCompleted_incrementsAndSetsStatus() {
		...
		assertThat(store.getPerGroupStatus()
		                .get(1L)).isEqualTo(PublishProgressStatusEnum.COMPLETED);
	}
	...
	void markFailed_incrementsAndSetsStatus() {
		...
		assertThat(store.getPerGroupStatus()
		                .get(2L)).isEqualTo(PublishProgressStatusEnum.FAILED);
	}
